namespace TraffickController.TrafficLight
{
    /// <summary>
    /// Datamodel for status from all trafficlights
    /// </summary>
    public class TrafficLightObject
    {
        public int A1 { get; set; } = 0;
        public int A2 { get; set; } = 0;
        public int A3 { get; set; } = 0;
        public int A4 { get; set; } = 0;
        public int AB1 { get; set; } = 0;
        public int AB2 { get; set; } = 0;
        public int B1 { get; set; } = 0;
        public int B2 { get; set; } = 0;
        public int B3 { get; set; } = 0;
        public int B4 { get; set; } = 0;
        public int B5 { get; set; } = 0;
        public int BB1 { get; set; } = 0;
        public int C1 { get; set; } = 0;
        public int C2 { get; set; } = 0;
        public int C3 { get; set; } = 0;
        public int D1 { get; set; } = 0;
        public int D2 { get; set; } = 0;
        public int D3 { get; set; } = 0;
        public int E1 { get; set; } = 0;
        public int EV1 { get; set; } = 0;
        public int EV2 { get; set; } = 0;
        public int EV3 { get; set; } = 0;
        public int EV4 { get; set; } = 0;
        public int FF1 { get; set; } = 0;
        public int FF2 { get; set; } = 0;
        public int FV1 { get; set; } = 0;
        public int FV2 { get; set; } = 0;
        public int FV3 { get; set; } = 0;
        public int FV4 { get; set; } = 0;
        public int GF1 { get; set; } = 0;
        public int GF2 { get; set; } = 0;
        public int GV1 { get; set; } = 0;
        public int GV2 { get; set; } = 0;
        public int GV3 { get; set; } = 0;
        public int GV4 { get; set; } = 0;
    }
}
